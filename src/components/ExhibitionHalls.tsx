...
export const ExhibitionHalls: React.FC<ExhibitionHallsProps> = ({
  ...
  return (
    <section id="exhibition-halls" className="w-full bg-[#0B1118] py-16 sm:py-20 border-b border-[#1E293B]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1 p-1 bg-[#111A24] border border-[#223142] rounded">
              <button
                type="button"
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-white bg-[#172230] hover:bg-[#1E2D3E] border border-[#2A3C4F] rounded transition-colors uppercase tracking-wider cursor-pointer"
            >
              Add Your Facility
            </button>
          </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8 pb-6 border-b border-[#1E293B]">
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium whitespace-nowrap rounded transition-colors cursor-pointer ${
                    isSelected
                      ? 'bg-white text-[#0B1118] font-semibold'
                      : 'bg-[#121B25] text-slate-300 hover:text-white hover:bg-[#172330] border border-[#1E2A38]'
                  }`}
                >
                  {sector}
                </button>
          ...
          <div className="relative min-w-[280px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-xs text-slate-200 bg-[#111A24] border border-[#223142] rounded placeholder-slate-500 focus:outline-none focus:border-[#FF6B00]"
            />
          </div>
        </div>
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="mt-4 pt-3 border-t border-[#1C2836] space-y-2">
                      {company.capabilities.slice(0, 2).map((cap, i) => (
                        <div key={i} className="flex items-baseline justify-between text-xs">
                          <span className="text-slate-400 truncate max-w-[170px]">{cap.name}:</span>
                          <span className="font-mono text-slate-200 text-[11px] font-semibold tabular-nums ml-2">
                            {cap.metric}
                          </span>
                        </div>
                      ))}
                    ...
                    <div className="mt-3.5 flex items-center gap-1.5 text-[11px] text-slate-400 font-mono">
                  ...
                  {/* Actions */}
                  <div className="mt-5 pt-4 border-t border-[#1C2836] flex items-center justify-between gap-3">
                    <button
                      type="button"
